...
export async function runSeries(
  ...
  for (const format of series.params.formats) {
    ...
    pieces.push(lead.id);
    if (!opts.preview) createPublishTask(ctx.db, series, lead, now);
  }
  ...
  return { pieces, scope: scope.label, formats: series.params.formats, note: opts.preview ? "Vorschau — die Rotation wurde nicht verbraucht." : "" };
}
