...
export function StoragePage() {
  ...
  return (
    <>
      ...
      {view.projects.map((p) => (
        <ProjektSpeicher key={p.projectId} p={p} busy={busy} loeschen={del} run={run} />
      ))}
    </>
  );
}
...
/**
 * Ein Projekt im Speicher.
 *
 * Eigene Komponente, weil jede Karte ihre eigene Seitenzahl braucht — und weil
 * 202 Zeilen mit je drei Knöpfen (423 insgesamt) keine Übersicht sind, sondern
 * eine Zumutung. Seit dem täglichen Aufräum-Job stehen hier ohnehin nur noch
 * Stücke, die jemand behalten will.
 */
function ProjektSpeicher({ p, busy, loeschen, run }: {
  p: StorageProject; busy: string | null;
  loeschen: (piece: StoragePiece, scope: "intermediates" | "recordings" | "all") => void;
  run: (label: string, fn: () => Promise<unknown>) => Promise<void>;
}) {
  const [open, setOpen] = useState<string | null>(null);
  const seiten = useSeiten(p.pieces, 25);
  return (
    <Card>
          ...
          <div className="mp-table-wrap"><table className="mp-table">
            ...
            <tbody>{seiten.aktuell.map((pc) => (
              <Fragment key={pc.pieceId}>
                ...
                {open === pc.pieceId && <tr><td colSpan={5}><ul className="mp-files">{pc.files.map((f) => <li key={f.path}><code className="mp-code">{f.path.split("/").slice(-1)[0]}</code> <span className="mp-label">{f.kind}</span> <span className="mp-num-cell">{fmtBytes(f.bytes)}</span>{f.assetId && <Button variant="danger" disabled={busy !== null} onClick={() => { if (window.confirm(`${f.path.split("/").slice(-1)[0]} löschen?`)) void run(f.assetId!, () => api(`/assets/${f.assetId}`, { method: "DELETE" })); }}>×</Button>}</li>)}</ul></td></tr>}
              </Fragment>
            ))}</tbody>
          </table></div>
