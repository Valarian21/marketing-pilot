 ...
 *
 * **Aufbau seit dem 22.09.2026: Kopf plus Reiter.** Vorher standen alle acht
 * Abschnitte untereinander; die Seite war über 4.000 px hoch, und die Frage
 * „wie steht es gerade" verlangte Scrollen bis zum Ende. Jetzt steht oben, was
 * immer gilt — die sechs Kennzahlen und der Zustand der Datenversorgung —, und
 * darunter wählt ein Reiter die Frage:
 *
 * - **Überblick**: Verlauf, der Weg vom Aufruf zum Kunden, was die Zahlen nicht sagen.
 * - **Herkunft**: wer war auf der Seite, woher kam er, wie viele legten ein Konto an.
 * - **Kanäle**: Publikum, je Kanal die Zahlen, Schnitt nach Sorte und Uhrzeit.
 * - **Beiträge**: welcher Beitrag was gebracht hat.
 * - **Datenlage**: kommen die Zahlen von selbst, und wie frisch sind sie.
 *
 * Der Reiter steht im URL-Hash, damit ein Neuladen dort bleibt, wo man war.
 *
...
import { Herkunft } from "../components/Herkunft.js";
...
const KURVEN = {
  ...
  besuche: { label: "Besuche der Seite", quelle: "produkt", hinweis: "Seitenaufrufe aller Quellen, vom Produkt selbst gezählt — auch Direktbesuche und Suchtreffer." },
...
/** Die Reiter. `id` steht im Hash, `label` im Knopf. */
const REITER = [
  { id: "ueberblick", label: "Überblick" },
  { id: "herkunft", label: "Herkunft" },
  { id: "kanaele", label: "Kanäle" },
  { id: "beitraege", label: "Beiträge" },
  { id: "daten", label: "Datenlage" },
] as const;
type ReiterId = typeof REITER[number]["id"];

const reiterAusHash = (): ReiterId => {
  const h = window.location.hash.replace(/^#/, "");
  return (REITER.find((r) => r.id === h)?.id ?? "ueberblick");
};
...
export function UebersichtPage() {
  ...
  const [reiter, setReiter] = useState<ReiterId>(reiterAusHash);

  // Zurück-Taste und ein von Hand gesetzter Hash sollen den Reiter wechseln.
  useEffect(() => {
    const auf = () => setReiter(reiterAusHash());
    window.addEventListener("hashchange", auf);
    return () => window.removeEventListener("hashchange", auf);
  }, []);
  const reiterWaehlen = (r: ReiterId) => {
    setReiter(r);
    history.replaceState(null, "", `${window.location.pathname}#${r}`);
  };
    ...
    <>
      ...
      <p className="mp-small mp-muted mp-stand">
        ...
        {/* Der Zustand der Versorgung gehört nach oben: eine Seite, die nur
            ...
            Der Verweis springt jetzt in den Reiter, nicht an eine Stelle der Seite. */}
        {view.versorgung.length > 0 && (
          <> · <button type="button" className={`mp-link-knopf${haengt.length ? " mp-stand-warn" : ""}`} onClick={() => reiterWaehlen("daten")}>
            {haengt.length === 0
      ...
      {/* Immer sichtbar: die sechs Zahlen, um die es geht. Ein Klick auf eine
          Kennzahl stellt das große Diagramm im Reiter „Überblick" darauf um. */}
      <div className="mp-kennzahlen">
        {(["aufrufe", "interaktionen", "klicks", "besuche", "konten", "zahlende"] as const).map((kid) => {
          const kz = k(kid);
          if (!kz) return null;
          ...
          return (
            <Kennzahl key={kid} label={kz.label} wert={kz.wert} davor={kz.davor}
              ...
              hinweis={kz.hinweis}
              aktiv={feld ? kurve === feld && reiter === "ueberblick" : false}
              {...(feld ? { onClick: () => { setKurve(feld); reiterWaehlen("ueberblick"); }, verlauf: punkte(feld) } : {})}
            />
          );
        })}
      </div>

      <nav className="mp-reiter" aria-label="Bereiche der Übersicht">
        {REITER.map((r) => (
          <button key={r.id} type="button" className={`mp-reiter-knopf${reiter === r.id ? " is-aktiv" : ""}`}
            aria-current={reiter === r.id ? "page" : undefined} onClick={() => reiterWaehlen(r.id)}>
            {r.label}
            {r.id === "daten" && haengt.length > 0 && <span className="mp-reiter-punkt" title={`${haengt.length} Kanal hängt`} aria-hidden="true" />}
          </button>
        ))}
      </nav>
        ...
        <>
            ...
            {tabelle ? (
              <div className="mp-table-wrap">
                <table className="mp-table">
                  <thead><tr><th>Tag</th><th className="mp-num-cell">Aufrufe</th><th className="mp-num-cell">Interakt.</th><th className="mp-num-cell">Reichweite</th><th className="mp-num-cell">Klicks</th><th className="mp-num-cell">Besuche</th><th className="mp-num-cell">Beiträge</th><th className="mp-num-cell">Neue Konten</th><th className="mp-num-cell">Umsatz</th></tr></thead>
                  <tbody>{[...view.verlauf].reverse().map((v) => (
                    <tr key={v.tag}>
                      ...
                      <td className="mp-num-cell">{zahl(v.besuche)}</td>
          ...
          <div className="mp-two-col">
            <Card>
              ...
              <p className="mp-small mp-muted">Die Stufen stammen aus verschiedenen Quellen und sind keine Kohorte: die Aufrufe sind Kanalzahlen, die Klicks zählt der Pilot selbst, Besuche und Konten meldet das Produkt. Der Trichter zeigt Größenordnungen, keine einzelne Nutzerreise.</p>
            </Card>
            <Card>
      ...
      {reiter === "herkunft" && (
        <>
          <Herkunft besucher={view.besucher} tage={view.zeitraum.tage} />
          <Card>
            <div className="mp-card-head">
              <h2>Besuche im Verlauf <span className="mp-muted mp-small">Seitenaufrufe je Tag, alle Quellen</span></h2>
            </div>
            <Tagesbalken punkte={punkte("besuche")} farbe="var(--mp-serie-3)" />
          </Card>
        </>
      )}
      ...
      {reiter === "daten" && <Versorgung zeilen={view.versorgung} />}
    </>
  );
}
